//
//  history_delete_true_t4_2.swift
...
class history_delete_true_t4_2: XCTestCase {
    ...
    func testExample() {
        ...
        self.provider.update(model: HistoryModel(text: "은행", date: Date()))
            .subscribe(onCompleted: {
                self.provider.delete(model: HistoryModel(text: "은행", date: Date()))
                    .subscribe(onCompleted: {
                        XCTAssertTrue(true)
                        expt.fulfill()
                    }) { error in
